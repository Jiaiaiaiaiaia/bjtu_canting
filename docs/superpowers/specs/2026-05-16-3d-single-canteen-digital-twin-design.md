...
- 术语：窗口选择称「窗口候选集 / 学生窗口分配」（`Canteen.shortest_window()`）。**不使用「路由」**——「路由」专指已删除的跨食堂 `StudentRouter`，不出现在本设计。
...
- **持久化全链**（否则现场可看、历史不可验）：`CampusCoordinator` 持 `interventions[]`、`snapshot()` 透出；`db_migrate` 仿现有幂等 `_column_exists`+`ALTER`（`db_migrate.py:26-34`）给 `campus_snapshot` 加 `interventions_json`；`_snapshot()`/`_compact_snapshot()` 带 `interventions`；`_flush_campus_snapshots()` INSERT、`_load_campus_history_rows()` SELECT+`json.loads` 同步；`get_campus_history`/`list_campus_history_configs` 经该函数自动传播。
...
- 干预：开窗后被后续 `shortest_window` 纳入；关窗停接新、不取消 pending request、原窗 drain；重复开/关 idempotent；拒关最后开放窗 `status:rejected`；事件入 `interventions_json`+history 全链可回读。
