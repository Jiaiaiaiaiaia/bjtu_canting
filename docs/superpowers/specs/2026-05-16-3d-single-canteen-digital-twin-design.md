...
**前端 — 模块拆分，3D 主体验**
- 前端模块根目录 `frontend/static/js/`（3D 模块在 `frontend/static/js/three/`）。`frontend/static/js/three/scene3d.js` 拆为 `scene3d.js`(core: renderer/相机/循环/`render(snapshot,state)`)、`canteen_scene.js`(多层堆叠/楼层聚焦/剖面/编码)、`intervention_ui.js`(窗口开关→干预 API→事件提示)、`state_adapter.js`(离散 snapshot→连续插值)，均置于 `frontend/static/js/three/`。**不做 campus_scene**。
...
- 「3D 默认主屏」为待实现目标（现状 `frontend/static/js/main.js:14` 的 `renderMode:'2d'`）；实现时改默认为 3D 并保 2D（`frontend/static/js/canvas_renderer.js`/SVG）兜底/调试。
...
- **前端主入口绑定**：3D 主体验的 preset-first 路径（`frontend/static/js/main.js` 的 `syncModeForms` / `loadDefaultCampusPreset` / `getCampusConfigForSubmit`，约 `main.js:147` / `175-187` / `203-214`）改调 `/api/campus/presets/single-canteen`；`/presets/default` 仅保留为旧校园联合演示/手动入口兼容，**不作为本次默认入口**——否则会把明湖/学四/学活待补带回，违反「单食堂唯一主线」（§1）。
...
- **干预即时可查（钉死）**：`_load_campus_history_rows()`（`campus_routes.py:150-172`）只读 DB、不读未 flush 的 `snapshot_buffer`；故干预 API 成功**或拒绝**后，必须**立即**追加一条 `event_type='intervention'` 的 compact snapshot 并**当场调用 `_flush_campus_snapshots()`**（不等 `STEP_FLUSH_THRESHOLD`），保证演示中干预后立刻 `GET /api/campus/history` 即可见该 intervention。该 intervention compact snapshot **沿用常规 compact 形状**（`campus_routes.py:106-115` 的 `campus_totals`/`canteens`/`in_transit`）+ `event_type='intervention'` + 本节链补的 `interventions` 字段，**不是新行结构**。
