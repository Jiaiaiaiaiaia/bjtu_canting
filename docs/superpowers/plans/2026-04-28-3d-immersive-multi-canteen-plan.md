...
def test_arrival_rate_formula():
    ...
    cfg = {"total_students": 28000, "lunch_alpha": 0.65,
           "coverage": 0.65, "peak_window_minutes": 90}
...
- [x] **Step 1: 统一 _session 加 mode 字段**
...
- [x] **Step 2: routes.py 现有响应都加 mode 字段**
...
#### 任务 A.10.2：Campus API history/statistics follow-up（2026-05-12 回写）

**Files:**
- Modify: `backend/api/campus_routes.py`
- Test: `backend/tests/test_campus_api.py`
- Spec: `docs/superpowers/specs/2026-04-28-3d-immersive-multi-canteen-design.md` §4.2 / §4.3.1 / §7.1

背景：A.10.1 初版已能把校园快照写入 `campus_snapshot`，但旧 `/api/history*` 仍只读 `simulation_snapshot`；`/api/campus/statistics` 初版返回 raw snapshot；`finish` 初版只按事件跨阈值采样，稀疏事件场景无法形成展示时间片 timeline。

- [x] **Step 1: 新增 campus history 专用入口**

新增：
- `GET /api/campus/history/configs`：从 `campus_snapshot` 汇总校园配置的 `snapshot_count / total_arrived / total_served`
- `GET /api/campus/history?config_id=...`：返回解析后的 `campus_totals / canteens / in_transit`

旧 `/api/history*` 保持 Phase 2 单食堂语义，不读 campus snapshot。

- [x] **Step 2: `/api/campus/statistics` 返回聚合统计**

返回 `avg_waiting_time / avg_service_time / avg_eating_time / avg_walk_time / switch_rate / window_served / seat_utilization / peak_queue_length / queue_timeline / seat_util_timeline / canteen_statistics`，不返回 raw snapshot。

- [x] **Step 3: `finish` 按 display tick 写 campus timeline**

`/api/campus/finish?display_tick_seconds=10` 在稀疏事件场景也应落 10、20、30 ... 的 step 快照，最后追加 `event_type="finish"`。

- [x] **Step 4: 保留 max_steps 防死循环保护**

`_advance_to_display_time()` 达到 `max_steps` 后不得再调用 `env.run(until=...)` 继续处理事件。

- [x] **Step 5: 回归验证**

```bash
PYTHONPATH=backend ./.venv/bin/python -m pytest backend/tests/test_campus_api.py -q
PYTHONPATH=backend ./.venv/bin/python -m pytest backend/tests -q
git diff --check -- backend/api/campus_routes.py backend/tests/test_campus_api.py
```

Observed: `11 passed` / `118 passed` / `diff --check` 无输出。

---

### A.11 食堂 preset 文件骨架（暂用占位数据）

#### 任务 A.11.1：3 个食堂 preset stub
...
> **占位数据**：5/4-5/5 实地调研后回填真实值。先用合理默认值跑通流程。明湖与学四已有调研数据，可直接填入。

- [x] **Step 1: 创建 3 个食堂 preset，按 spec §3.2 schema**

每个文件初始可用以下数据（明湖、学四基于已有调研，学活待 5/4-5/5 补）：
- `minghu_xueyi.json`：3 层（1F 6 窗口/172 座；2F 13 窗口/272 座；3F 14 窗口/290 座；avg_serve_time_seconds=30）
- `xuehuo.json`：占位单层（5 窗口 / 150 座位），调研后回填
- `xuesi.json`：2 层（1F 9 窗口/150 座；2F 9 窗口/150 座；avg_serve_time_seconds=36；备注：二楼面食排队 10 分钟+）

每个 preset 都加注释字段 `"_TODO_field_research_pending": true`。

- [x] **Step 2: 创建 _campus.json 全局配置 stub**
...
git commit -m "data: 3 食堂 preset 骨架（占位数据，5/4 实地调研后回填）"
```

Status: A.11 只完成占位骨架。真实调研数据闭环仍属于 A.17：回填真实 `presets/*.json` 与 `_campus.json walking_time_seconds`，并删除 `_TODO_field_research_pending`。
...
- Add: `backend/tests/test_frontend_main_js_contract.py`

参考 spec §6.1。

> **关键不破坏原则**：所有 `drawWindows / drawSeats / drawStudentDots` 函数体一行不动。只改控制流与 state 形状。

- [x] **Step 1: 启动 dev server，记录 Phase 2 单食堂模式行为**
...
- [x] **Step 2: 加 namespace 头与 state 扩展**
...
Status: A.12 已完成并提交 `45a2266`。新增静态契约测试锁住 `window.CanteenApp`、state 字段、`dispatchStep` mode 分派和绘图 helper 暴露；验证命令包括 `node --check frontend/static/js/main.js`、`PYTHONPATH=backend ./.venv/bin/python -m pytest backend/tests -q`（122 passed）和本地服务单食堂接口流。
...
- Add: `backend/tests/test_frontend_campus_js_contract.py`

参考 spec §6.1 campus.js 关键实现。

- [x] **Step 1: 写 campus.js IIFE 框架**

挂 `window.CanteenApp.refreshCampusView` / `fillCanteenSelect` / `updateCampusOverview`。

- [x] **Step 2: 实现 fillCanteenSelect（防抖 lastCanteenOrderKey）**

参考 spec §6.1 fillCanteenSelect 完整代码。

- [x] **Step 3: 实现 refreshCampusView**

参考 spec §6.1 refreshCampusView 完整代码（含三层联动 + 兜底初始化 activeCanteenId）。

- [x] **Step 4: 实现 filterByFloor**

参考 spec §6.1 filterByFloor 代码。注意 waiting_queue_length 取 canteen 级，不按楼层过滤。

- [x] **Step 5: 实现 updateCampusOverview**

读 `snapshot.campus_totals.*` 更新 5 个 DOM 节点。

- [x] **Step 6: 在 index.html 底部加 `<script src="...campus.js">` 在 main.js 之后**

- [x] **Step 7: 手测：必须能加载且 activeCanteenId 切换不报错**

需要等 A.14 + A.15 完成才能完整测；这一步只验证 JS 加载成功（控制台无 error）。

Status: A.13 已完成并提交 `79f58bc`。实现 `campus.js` 食堂下钻层，含 `refreshCampusView` / `fillCanteenSelect` / `pickCanteenView` / `filterByFloor` / `updateCampusOverview`；在 A.15 DOM 尚未加入时做空值保护，保证脚本现在即可加载。验证命令包括 `node --check frontend/static/js/campus.js`、前端契约测试、全量 `PYTHONPATH=backend ./.venv/bin/python -m pytest backend/tests -q`（125 passed）和本地服务页面/script 加载检查。
...
- Add: `backend/tests/test_frontend_campus_map_js_contract.py`

参考 spec §6.1 campus_map.js 关键实现。

- [x] **Step 1: index.html 加 SVG 容器**
...
- [x] **Step 5: 在 index.html 加 `<script src="...campus_map.js">` 在 campus.js 之后**

- [x] **Step 6: 手测：跑校园模式，看 SVG 渲染正确**

需要后端先有可跑的校园模式数据；先用 mock JSON 测试也可。

Status: A.14 已完成并提交 `df0a77c`。实现 `campus_map.js` SVG marker、队列热度、in-transit 圆点和食堂点击下钻；`index.html` 已加入隐藏 SVG 容器并按 `main.js -> campus.js -> campus_map.js` 顺序加载。验证命令包括 `node --check frontend/static/js/campus_map.js`、前端地图契约测试、全量 `PYTHONPATH=backend ./.venv/bin/python -m pytest backend/tests -q`（127 passed）和本地服务页面/script 加载检查。

- [x] **Step 7: Commit**

```bash
git add frontend/static/js/campus_map.js frontend/templates/index.html
git commit -m "feat(frontend): campus_map.js 校园地图 SVG（3 食堂热度 + 在路上学生）"
```
...
- Add: `backend/tests/test_frontend_floor_tabs_js_contract.py`

参考 spec §6.1 floor_tabs.js 关键实现。

- [x] **Step 1: index.html 加楼层 Tab 容器**
...
- [x] **Step 3: 实现 makeTab + 事件绑定**

按钮点击设 `state.activeFloorId` + 立即调 `refreshCampusView(state.lastData)`。

- [x] **Step 4: 在 index.html 加 `<script src="...floor_tabs.js">` 在 campus.js 之后**

- [x] **Step 5: 手测：双层食堂切换 Tab 应正确过滤窗口/座位**

- [x] **Step 6: Commit**

Status: A.15 已完成并提交 `68b3c02`。实现 `floor_tabs.js` 楼层 Tab 层，单层食堂自动清空 Tab 与楼层过滤，多层食堂渲染“全楼层 / nF”按钮，点击后更新 `activeFloorId` 并触发 `refreshCampusView` 重绘。验证命令包括 `node --check frontend/static/js/floor_tabs.js`、前端契约测试、全量 `PYTHONPATH=backend ./.venv/bin/python -m pytest backend/tests -q`（131 passed）和本地服务页面/script 加载检查。
...
- Modify: `frontend/static/js/main.js`（A.16 UI 控制接线）

参考 spec §6.1 index.html 增量。

- [x] **Step 1: 参数配置页加运行模式 radio + 校园配置表单**

参考 spec §6.1 HTML 示例。`#campus-mode-form` 默认 hidden，radio 切换显示。

- [x] **Step 2: 仿真运行页加视图切换 toggle**
...
- [x] **Step 6: 手测：单食堂 / 校园两模式 UI 都能正常切换显示**
...
- [ ] **Step 2: 5/4 周一中午（11:30-13:00）调研 2 个食堂**
  - 明湖学一 + 学活食堂（明湖前面已经调研过基础数据，本次主要补 5 人服务时长 + 高峰队伍 + 平面图）
...
- [ ] **Step 4: 5/5 周二中午调研学四食堂 + 步行时间补齐**
  - 学四：补充确认（基础数据已采集，重点验证窗口数与服务速度）
  - 步行时间补齐：学四↔明湖、学四↔学活、入口↔学四

- [ ] **Step 5: 回填 3 个 preset JSON 与 _campus.json walking_time_seconds**
...
cd backend && python app.py &
```
浏览器访问，提交校园配置（用真实 3 食堂 preset），开始仿真，看是否合理。

- [ ] **Step 7: Commit 调研数据**

```bash
git add backend/simulation/presets/*.json
git commit -m "data: 北交大主校区 3 食堂实地调研真实参数（5/4-5/5）"
```
...
**目标**：单食堂模式 100% 与 Phase 2 一致 + 校园模式 3 食堂能跑全流程，无 console 报错。
...
看：
- 校园地图视图：3 食堂方块 + 热度颜色变化 + 在路上学生小点移动
...
def run(exp):
    # 用真实 3 食堂 preset，跑一次校园仿真，返回 4 项指标
...
- [ ] **Step 2: 实现 3 食堂建筑（按 campus_position 摆放）**
...
- [ ] **Step 5: 手测：跑校园模式应看到 3 个建筑 + 在路上学生**

- [ ] **Step 6: Commit**

```bash
git add frontend/static/js/three/campus_view.js
git commit -m "feat(3d): campus_view.js 校园 3D（3 食堂建筑 + 热度 + 在路上学生）"
```
...
- [ ] 校园模式手测：3 食堂联合跑通、视图切换、楼层 Tab、跨食堂迁移可观察
- [ ] 3 食堂 preset 含真实调研数据（无 `_TODO_field_research_pending`）
- [ ] 6 份交付物按命名规范提交
...
| 版本 | 日期 | 变更 | 作者 |
|---|---|---|---|
| v1.0 | 2026-04-28 | 初稿：Phase A 20 任务 + Phase B 11 任务 | 朱思思 |
| v1.1 | 2026-04-28 | plan-reviewer 4 条修订（食堂命名 / test 数 / A.6 循环依赖 / OrbitControls 单独下载） + advisory（A.5 stats 5 条 / A.10 step 7 ≥80） | 朱思思 |
| v1.2 | 2026-04-28 | 收尾：A.2.2 测试数对齐 / A.6 step 编号 | 朱思思 |
| v1.3 | 2026-04-28 | 同步 spec v1.6 缩小建模范围（4→3 食堂，去掉学苑）：A.11.1 preset 数与文件名 / A.14.1 SVG 食堂标记数 / A.17.1 调研食堂数量与步行对数 / A.18.1 联调验收数 / B.3.1 3D 校园建筑数 / DoD 对应数字。campus config 示例 coverage 默认 0.78→0.65。 | 朱思思 |
| v1.4 | 2026-05-12 | 回写 A.10/A.11 执行状态：A.10.1/A.11.1 checkbox 标记完成；新增 A.10.2 campus history/statistics follow-up 任务，记录 `/api/campus/history*`、statistics 聚合、finish display-tick timeline 与 max_steps 保护；明确 A.11 真实数据仍留到 A.17。 | 朱思思 |
| v1.5 | 2026-05-12 | 回写 A.12 执行状态：main.js 已加 `window.CanteenApp` namespace、控制层 state 字段、`dispatchStep` mode 分派和单食堂保形验证；新增前端静态契约测试。 | 朱思思 |
| v1.6 | 2026-05-12 | 回写 A.13 执行状态：新增 `campus.js` 食堂下钻层、空值保护、楼层过滤、校园总览字段更新和前端契约测试；确认脚本加载顺序为 `main.js` 后接 `campus.js`。 | 朱思思 |
| v1.7 | 2026-05-12 | 回写 A.14 执行状态：新增 `campus_map.js` SVG 校园地图层、3 食堂 marker、队列热度、in-transit 点和脚本顺序契约测试。 | 朱思思 |
| v1.8 | 2026-05-12 | 回写 A.15 执行状态：新增 `floor_tabs.js` 楼层 Tab 层、单层清空、多层切换、activeFloorId 重绘和前端契约测试。 | 朱思思 |
